...
* ``method='mcf'`` (default): global max-weight vertex-disjoint path cover via min-cost
  flow over consecutive + bridge edges, solved by the package's deterministic sparse
  bipartite matcher. Solves the panel jointly, so a persistent cluster's identity is routed
  *around* a transient merge/split instead of handed off locally. Recommended for labelling.
...
Entry points: ``analyze_cluster_lineage(covar_data, ...) -> RiskClusterReport`` and the
convenience wrapper ``run_cluster_lineage_report(...) -> (figures, tables)``. Both and the
two public configuration/report containers are exported from ``factorlasso``.
...
def _validate_edges(
    ...
    """Validate matching dimensions and the ordered positive-weight edge list."""
    if n_left < 0 or n_right < 0:
        raise ValueError("matching dimensions must be non-negative")
    pairs = set()
    for left, right, weight in edges:
        if not 0 <= left < n_left:
            raise ValueError(f"left index {left} is outside [0, {n_left})")
        if not 0 <= right < n_right:
            raise ValueError(f"right index {right} is outside [0, {n_right})")
...
def solve_max_weight_matching(
    ...
    """Solve sparse maximum-weight bipartite matching with free unmatched nodes.

    The ordered edge list receives the deterministic perturbation
    ``-k * 1e-12 * max_weight`` before optimisation. SciPy's sparse LAPJVsp assignment
    implementation solves an equivalent successive-shortest-augmenting-path problem. One
    private dummy column per left vertex leaves unmatched vertices free, while only strictly
    positive adjusted real edges can enter the returned matching.
    ...
    edges : list of tuple
        Ordered ``(left, right, weight)`` triples with finite positive weights.

    Returns
    -------
    dict
        Mapping from matched left indices to distinct right indices.

    Raises
    ------
    ValueError
        If dimensions, endpoints, weights, or edge uniqueness are invalid.
    """
    _validate_edges(n_left, n_right, edges)
    ...
    for edge_index, (left, right, weight) in enumerate(edges):
        adjusted_weight = weight / max_weight - edge_index * 1e-12
        if adjusted_weight > 0.0:
            rows.append(left)
            columns.append(right)
            values.append(adjusted_weight)
    rows.extend(range(n_left))
    columns.extend(n_right + left for left in range(n_left))
    values.extend([np.nextafter(0.0, 1.0)] * n_left)
    biadjacency = coo_matrix(
        (values, (rows, columns)), shape=(n_left, n_right + n_left)
    ).tocsr()
    left_indices, right_indices = min_weight_full_bipartite_matching(
...
def _match_panel_mcf(snapshots: Dict[pd.Timestamp, Dict[Any, _Fingerprint]],
    ...
    clustered_at = {
        d: set().union(*members_at[d].values()) if members_at[d] else set()
        for d in dates
    }
    ...
    matching = solve_max_weight_matching(
        N,
        N,
        [
            (node_index[u], node_index[w], float(int(round(wt * scale))))
            for u, w, wt in edges
            if int(round(wt * scale)) > 0
        ],
    )
...
def _build_tracks(relabel: pd.DataFrame,
    ...
    for did, grp in relabel.groupby('derived_id'):
        ...
        for _, row in grp.sort_values('date').iterrows():
            ...
            fv.append(np.sqrt(fp.factor_var))
            iv.append(np.sqrt(fp.idio_var))
            tv.append(np.sqrt(fp.total_var))
            r2.append(fp.r2)
            sz.append(len(fp.members))
            dom.append(fp.dominant)
...
def _classify(tracks: Dict[str, TrackPanel], x_covars: Dict[pd.Timestamp, np.ndarray],
    ...
    classification : pandas.DataFrame
        One classification row per track, sorted by coverage.
    transitions : pandas.DataFrame
        One transition row per beta-regime break.
    """
    rows, trans = [], []
...
class RiskClusterReport:
    ...
    def label_tracks(self, asset_meta: pd.DataFrame, *,
        ...
        """
        sac = asset_meta[sub_class_col]
        acl = asset_meta[class_col]
        labels: Dict[str, str] = {}
    ...
    def to_figures(self):
        ...
        Notes
        -----
        Matplotlib is optional and imported only when this method is called; it is not a
        factorlasso runtime dependency.

        Returns
        -------
        list
            Matplotlib figures for the lineage timeline and factor-vol heatmap.
        """
        ...
        ax.set_title('derived cluster lineage timeline')
        figs.append(fig)
        ...
        ax2.set_yticklabels(ids, fontsize=6)
        ax2.set_title('factor vol by derived cluster')
        fig2.colorbar(im, ax=ax2)
        figs.append(fig2)
        return figs


def analyze_cluster_lineage(covar_data: RollingFactorCovarData, *,
    ...
    covar_data : RollingFactorCovarData
        Rolling factor covariance data, one snapshot per date.
    overlap_metric : str, default='overlap'
        ``'overlap'`` coefficient or ``'jaccard'`` for membership affinity.
    beta_metric : str, default='mahalanobis_sf'
        Recorded in ``params`` for provenance; beta distance is always spread volatility
        under the average factor covariance.
    combine : str, default='gated'
        ``'gated'`` arbitration of overlap and beta, or ``'blend'``.
    overlap_band : tuple of float, default=(0.15, 0.60)
        ``(low, high)`` gate thresholds. Overlap at least ``high`` links outright;
        mid-band overlap links only within ``spread_vol_cut``; lower overlap never links.
    spread_vol_cut : float, default=0.015
        Beta-spread-vol threshold in annualised fractional-vol units.
    bridge_window : int, default=6
        Number of dates a dormant track may be revived across.
    bridge_decay : float, default=0.5
        Per-date weight decay on a bridged link for the ``'mcf'`` matcher.
    w_overlap : float, default=0.6
        Weight on overlap when overlap and beta are blended.
    weighting : str, default='equal'
        ``'equal'`` or ``'inv_vol'`` member weighting in a fingerprint.
    taxonomy : TaxonomyConfig, optional
        Classification thresholds; defaults to ``TaxonomyConfig()``.
    method : str, default='mcf'
        ``'mcf'`` or ``'graph'`` selects the deterministic sparse matcher; any other value
        selects the per-transition Hungarian matcher.

    Returns
    -------
    RiskClusterReport
        Relabel map, track panels, classification, lineage, transitions and parameters.
    """
...
def run_cluster_lineage_report(covar_data: RollingFactorCovarData, **kwargs):
    """Run the cluster analysis and return its ``(figures, tables)`` pair."""
    report = analyze_cluster_lineage(covar_data, **kwargs)
