...
def compute_ewm_covar_newey_west(a: np.ndarray,
    ...
    """
    Newey–West HAC-adjusted EWMA covariance estimator.

    Adds autocovariance adjustments for up to *num_lags* lags using
    ...
    a : np.ndarray, shape (T, N)
        Demeaned return matrix.
    num_lags : int, default 2
        Number of autocovariance lags.
    span : int, optional
        EWMA span.
    ewm_lambda : float, default 0.94
        Decay factor.
    ...
    np.ndarray, shape (N, N)
    """
    ewm0 = compute_ewm_covar(a=a, span=span, ewm_lambda=ewm_lambda, is_corr=False)

    if num_lags > 0:
        nw = np.zeros_like(ewm0)
        for m in range(1, num_lags + 1):
            a_m = np.empty_like(a)
            a_m[m:] = a[:-m]
            a_m[:m] = np.nan
            ewm_m = compute_ewm_covar(a=a, b=a_m, span=span, ewm_lambda=ewm_lambda)
            nw += (1.0 - m / (num_lags + 1)) * (ewm_m + ewm_m.T)
        result = ewm0 + nw
    else:
        result = ewm0
    ...
    if is_corr:
        ...
        if np.nansum(d) > 1e-10:
            inv_vol = np.reciprocal(np.sqrt(d))
            result = result * np.outer(inv_vol, inv_vol)
        else:
            result = np.identity(a.shape[1])
...
def squeeze_covariance_matrix(covar: Union[np.ndarray, pd.DataFrame],
    ...
    """
    Shrink small eigenvalues toward a floor to regularise covariance matrices.

    Eigenvalues below ``squeeze_factor × max(eigenvalue)`` are raised to
    that floor.  Optionally rescales to preserve original diagonal variances.

    Parameters
    ----------
    covar : array-like, shape (N, N)
        Input covariance matrix.
    squeeze_factor : float, optional
        Fraction of the largest eigenvalue used as a floor.  ``None`` or 0
        disables squeezing.
    is_preserve_variance : bool, default True
        If True, rescale so that diagonal variances match the original.
    ...
    References
    ----------
    Sepp et al. (2024), "Squeezing Financial Noise: A Novel Approach to
    Covariance Matrix Estimation", SSRN 4986939.
    """
    ...
    if isinstance(covar, pd.DataFrame):
        ...
        cov_pd = pd.DataFrame(covar)

    variances = np.diag(cov_pd.to_numpy())
    good = np.where(np.logical_and(np.greater(variances, 0.0), ~np.isnan(variances)))[0]
    good_tickers = cov_pd.columns[good]
    clean = cov_pd.loc[good_tickers, good_tickers].to_numpy()

    eigenvalues, eigenvectors = np.linalg.eigh(clean)
    floor = squeeze_factor * np.max(eigenvalues)
    squeezed_eig = np.maximum(eigenvalues, floor)
    squeezed = eigenvectors @ np.diag(squeezed_eig) @ eigenvectors.T

    if is_preserve_variance:
        ratio = np.sqrt(np.diag(clean) / np.diag(squeezed))
        squeezed = squeezed * np.outer(ratio, ratio)

    result_pd = pd.DataFrame(squeezed, index=good_tickers, columns=good_tickers)
    result_pd = result_pd.reindex(index=cov_pd.columns, columns=cov_pd.columns).fillna(0.0)

    if isinstance(covar, np.ndarray):
        return result_pd.to_numpy()
    return result_pd
