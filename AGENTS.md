...
- The local supplement is `docs/documentation_standard.md`: Markdown methodology articles with
  the heading `Implementation in factorlasso`, RST retained for `api.rst` and the older utility
  pages, numpydoc docstrings, the admissible reference sources, and the figure allowlist.
- `tools/docs_inventory.json` assigns every name in `factorlasso.__all__` to exactly one
  methodology article. Adding a public export means assigning it to an article in the same
  change; otherwise `tools/check_docs.py` and `tests/test_docs_tooling.py` fail. An article is
  marked `adopted` only after its example passes, its references are verified and its rendering
  has been inspected.
...
- The remaining documentation plan is `agents/ROADMAP_DOCUMENTATION.md`.
