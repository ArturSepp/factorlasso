...
\pkg{group-lasso} \citep{Moe:2020}, \pkg{asgl}
...
\pkg{oem} \citep{Huling+Chien:2022} cover overlapping subsets of this
...
second-order cone programme solvable by CLARABEL
\citep{Goulart+Chen:2024}, ECOS \citep{Domahidi+Chu+Boyd:2013}, or SCS
\citep{ODonoghue+Chu+Parikh+Boyd:2016} via
...
configurations ($\tau = 1.0$) sit at $0.83$ to $0.85$. The
ungated group estimators sit above them at $0.91$ to $0.92$
because they zero whole groups
...
The replication materials live in the \pkg{factorlasso} repository
under the \code{papers/jss\_2026/} directory and in the replication
archive submitted with this manuscript. We
...
\end{CodeChunk}
%
The \code{[simulations]} extra brings in \pkg{PyYAML} \citep{pyyaml}
for study configuration, \pkg{joblib} for parallel cell execution,
\pkg{pyarrow} \citep{pyarrow} for parquet output, and \pkg{SCS} and
\pkg{ECOS} as
