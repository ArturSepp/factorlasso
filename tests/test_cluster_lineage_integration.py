...
def _hungarian(panel: RollingFactorCovarData) -> RiskClusterReport:
    ...
    return analyze_cluster_lineage(
        panel,
        method="hungarian",
...
def test_run_report_wrapper_returns_figures_and_tables(
    ...
    figures, tables = run_cluster_lineage_report(persistent_panel, method="hungarian")
    assert len(figures) == 2
...
def test_custom_taxonomy_exercises_classification_thresholds(
    ...
    report = analyze_cluster_lineage(
        persistent_panel,
        method="hungarian",
        taxonomy=taxonomy,
        weighting="equal",
