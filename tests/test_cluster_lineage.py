...
* ``_match_panel_mcf`` (``method='mcf'``, the default) solves the whole panel jointly, so a
  cluster absent for one date keeps its identity across the gap and is tagged ``bridge``. Its
  deterministic sparse solver is part of the core package, so the default works without an
...
# the defaults analyze_cluster_lineage passes down, restated so a case can vary one of them
...
# --------------------------------------------------------------------------- #
# _match_panel_mcf: the dependency-free global min-cost-flow matcher
# --------------------------------------------------------------------------- #
def run_mcf(snapshots, bridge_window: int = 1):
    """Run the deterministic min-cost-flow matcher with the default link settings."""
...
def build_report(method: str = 'hungarian') -> RiskClusterReport:
    ...
    return analyze_cluster_lineage(make_rolling(), method=method)


def test_analyze_cluster_lineage_tracks_a_stable_panel_end_to_end() -> None:
...
def test_analyze_cluster_lineage_agrees_between_the_two_matchers_on_a_stable_panel() -> None:
